...
import numpy as np  
from mod.dsa.neld_fun_0.main_0_help import get_navs_bar
from mod.dsa.neld_fun_0.help_funn import safe_path_join

from  mod.dsa.neld_fun_0.main_0_help import box_style,dropdown_style,dropdown_options_style
...
from neld_data_all import general_nam_main,file_path_datab,file_path_orgb,github_link,navbar

from mod.dsa.neld_fun_0.help_model_pred import model_pred 
from mod.dsa.neld_fun_0.get_path import get_data_mode  
from mod.dsa.apps.side_bar import  dnn_page
...
def algorithm_param(file_path_org,
    ...
    list_features=kward.get('list_features')
    base_features_list=kward.get('base_features_list')
...
#  [ 'result',]+
    ...
    param_input=['model-pred', 'Morphologic Param','skl_shaft_pred','Skeleton','rhs',
                 'model_shap','cylinder_heatmap','riplet','graph_center','clean_path_dir', ]
    # param_input=['dyna param','get_training']
    param_inputbv=['dyna param','Smooth','Resizing','get_training', 'intensity_rhs','intensity_all','iou','roc','dash_pages', 'clean_path_dir', 
                    'annotations', 'model_pred' ,'skl','neld_names_all',]
    param_input_save=[ 'spines_segss_old','spines_segss_old_2', 'clean_path_dir',]
    param_list=['tname','file_path_org','param_dropdown','param_input','param_all', 'path_dirs_weig', 'param_list','param_dropdowni','file_path_data','param_inputbv','path_list','head_navbar','path_heads_show_nami',]
    param_all=[]
    ...
    param['riplet']['param']=dict( 
                                den_Circon=10,
                                size_monte=500, 
                                N_plot=40,

                                    )      
    param['riplet']['param_fix']=dict( 
                                    N_interpolate=1000, 
                                    )   
...
class algorithm:
    ...
    def pre_test_train( self, 
        ...
        modes={
                val:{  
                        name:{
                            ...
                            for name in model_sufix_show+['mode0']
                        }  
        ...
        for val in path_heads_show:   
                # ids,name,dirs=0,'mode0',path_dirs_show[0].split('_')[0]
                ...
                # for dirs in [mm.split('_')[0] for mm in path_dirs_show]:
                for dirs in path_dirs_show:
                    data_dir_tmp=data_dir if data_dir is not None else dirs
 ...
 '''
    ...
    def test( self,   
        ...
        pinn_dir_data_all=pinn_dir_data_all if pinn_dir_data_all is not None else dmode.pinn_dir_data_all
        for ixi,mode_id in enumerate(mode_ids):#data_mode.keys(): # 
            ...
            neld_cla=model_pred(   
                                ...
                                path_display_dic=path_display_dic,
                                # data_studied=param["action"]["param"],
                                # path_display=path_display,
            ...
            if param['model-pred']['tf']:   
                ...
                if model_type.startswith(('vol','cnn')):
                    ...
                    neld_cla.get_shaft_pred_linear(
                                            # weights=weights,
                                            path_dict=path_dict,
                                            train_spines=train_spines,
                                            model_type=model_type,
                    ...
                    neld_cla.get_shaft_pred(
                                            # weights=weights,
                                            train_spines=train_spines,
                                            model_type=model_type,
                                            **param['model-pred']['param']
                                        )    
                neld_cla.get_shaft_process(
                                        # weights=weights,
                                        train_spines=train_spines,
                                        model_type=model_type,
                                        **param['model-pred']['param']
            ...
            if param['riplet']['tf']: 
                neld_cla.get_riplet(**param['riplet']['param'])
    ...
    def train( self,  
        ...
        for ixi,mode_id in enumerate(mode_ids):#data_mode.keys(): #
            ...
            size_threshold=param['model_pred']['param']['size_threshold'] 
            train_spines=True #data_mode[mode_id]['train_spines']
            if param['get_training']['tf']:
                if model_type.startswith(('cml','ML','cML')): 
                    neld_cla.train_model_spine_cml(     
                                            model_sufix=model_sufix, 
                                            train_spines=train_spines, 
                                            model_type=model_type,  
                                            num_sub_nodes=20000,
                    ...
                    neld_cla.train_model_spine_gcn(
                                            train_spines=train_spines, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
                    ...
                    neld_cla.train_model_spine_kal(
                                            path_dict=path_dict,
                                            train_spines=train_spines, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                    ...
                    neld_cla.train_model_spine_PINN(
                                            train_spines=train_spines, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
                    ...
                    neld_cla.train_model_spine_pnet(
                                            train_spines=train_spines, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
                    ...
                    neld_cla.train_model_spine_cnn(
                                            train_spines=train_spines, 
                                            model_type=model_type, 
                                            model_sufix=model_sufix, 
                                            entry_names=entry_names,
            ...
            for entry_name,exit_name,old_path in zip(save_entry_exit['entry'],save_entry_exit['exit'],save_entry_exit['old_path']):
                for paath in ['entry','old']:
                    ...
                    if param['Skeleton']['tf']: 
                        ...
                        print('[[[[[[[--------------skl------------]]]]]]]',dict_wrap )
                        neld_cla.get_wrap(wrap_part='shaft', 
                                        alpha_fraction=1,
                                        offset_fraction=.7,
                                        entry_name=entry_name,
                                        exit_name=exit_name,
