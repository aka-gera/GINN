...
class get_files(get_name,get_param):
    def __init__(self,
        ...
        if data_mode is not None:
            ...
            self.pre_portion=pre_portion if pre_portion is not None else neld_data['data_studied']
    ...
    def get_names_path(self,**kward):
        ...
        print('[[[[[[self.data_studied]]]]]]',self.file_path_org,file_path_org,self.data_studied,self.file_path_data,'lol0')
    ...
    def get_model_opt_name(self,  
        ...
        if file_path_model is None:
            file_path_model = os.path.join(file_path_org ,  'model',model_type,) 
        ...
        self.model_dir_path = {
            'spine': {
                ...
                **model_paths(self.file_path_model, 'spine', mo,mlname),
                'oloss': os.path.join(self.file_path_model, 'oloss_spine.txt'),
            ...
            'shaft': {
                ...
                'oloss': os.path.join(self.file_path_model, 'oloss_shaft.txt'),
    ...
    def update_nam_gen_dropdown(self,name=None,file_path_org=None,obj_org_path=None,path_file_dir=None): 
        ...
        print(f'[[[[[[[[[[[[[[]]]]]]]]]]]]]]{data_shaft_path}++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++',nam_gen_show)
        file_path_model=  os.path.join(file_path_org,'model')
        path_heads_show=[mm for mm in os.listdir(file_path_model) if mm not in ['.DS_Store',] 
                        and os.path.isdir(os.path.join(file_path_model, mm))]
        # file_path_model_data__=  os.path.dirname(self.file_path_model_data)
...
'''
 ...
 '''
        ...
        # self.obj_org_path_old=os.path.join(self.obj_org_path, self.neld_name,data_org )
        # self.obj_org_path_old=os.path.join(os.path.dirname(os.path.dirname(self.obj_org_path)), self.neld_name,data_org )
        self.file_path_model_data_old=os.path.join(os.path.dirname(os.path.dirname(self.file_path_model_data)), self.neld_name,data_org )
        ...
        self.neld_path_org_old=os.path.join(self.obj_org_path, self.neld_name, data_org) 
...
from mod.dsa.neld_fun_0.get_path import get_path_train,get_name,get_model_name
