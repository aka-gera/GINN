...
import mod.kalman.lorenz.neld_fun_0.help_funn as hff   


from mod.kalman.lorenz.neld_fun_0.get_path import assign_if_none,get_name,get_param,get_files
    ...
    def get_train_input_dnn(self,  
        ...
        for entry_name in entry_names:
            for neld_name in neld_names: 
                ...
                if tf_train:
                    # if pre_portion=='head': 
                    #     pathh=os.path.join( self.neld_path_true_final ,'intensity_hmod.kalman.lorenz.neck_head.txt')
                    # elif pre_portion=='neck_head': 
                    #     pathh=os.path.join( self.neld_path_true_final ,'intensity_hmod_neck_head.txt')
                    # else:
                    #     pathh=os.path.join( self.neld_path_true_final ,'intensity_hmod_smod.kalman.lorenz.txt')
                    # rhs.append(tf.cast(
 ...
 '''
    ...
    def get_hmod_pred_kal(self, 
        ...
        for  index,neld_name in enumerate(neld_names):
            ...
            for kk,tyy in zip(['initial'],smods_logit[:1]):  
                    rhs00=save['approx']
            ...
            ku='true'
            smods_logit=  self.intensity_logit_dict[ku]
            for kk,tyy in zip(['true','obs'],smods_logit): 
                rhs0=save[kk]   
                np.savetxt(self.path_file_sub[tyy][key], rhs0, fmt='%f') 
                if kk =='true':
                    losst+=loss_fn(rhs00,rhs0)    
            ...
            if disp_infos:
                ...
                print(f'data stores in: {smod_portion_path}     {path_2}')  
    ...
    def model_shap(self, 
        ...
        if  model_type.startswith(('cml','ml',)):
            ...
            if model_type.startswith(('gcn',)):
                from mod.kalman.lorenz.neld_fun_0.help_gcn_one_hot import model_choice
                adj_tf=True 
            elif model_type.startswith('dnn'):
                from mod.kalman.lorenz.neld_fun_0.help_dnn_one_hot import model_choice 
                adj_tf=False 
            elif model_type.startswith(('cnn','vol',)):
                from mod.kalman.lorenz.neld_fun_0.help_vol_one_hot import model_choice 
                mmnn=['vol']
