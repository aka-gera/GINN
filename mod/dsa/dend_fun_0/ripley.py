...
def get_distance(save, list_index=None):
    ...
    for i in range(N):
        ...
        d, path = nx.single_source_dijkstra(G, src,weight='weight') 
        # intensities['idx'][src]['distG']=d
        # intensities['idx'][src]['pathG']=path
        # {mm:[] for mm in ['dist','path']}

        for j in range(i+1, N):
            dist[i, j] = dist[j, i] = d[list_index[j]]
            intensity[path[list_index[j]]]=dist[i,j]
            intensities['path'].append(path[list_index[j]])
...
def monte_carlos_ripley_incremental(
    ...
    for ii in range(size_monte): 
        ...
        dist,_ = get_distance(save, list_index=list_index)
