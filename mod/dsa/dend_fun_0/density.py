...
def get_chi(pvals,eps = 1e-12):
    
    pvals = np.clip(pvals, eps, 1)  
    X = -2 * np.sum(np.log(pvals))
    df = 2 * len(pvals)
...
class get_save:
    def __init__(self):
        self.density_heatmap,self.density_heatmap_points,self.density_heatmap_points_org,self.density_heatmap_surface,self.density_org_points=None,None,None,None,None

class  get_cylinder(graph_cylinder_heatmap):     
    ...
    def __call__( self,save_data=True):
        self.gsave=get_save() 
        ...
        self.gsave.density_heatmap,self.gsave.density_heatmap_points,self.gsave.density_heatmap_points_org,self.gsave.density_heatmap_surface,self.gsave.density_org_points \
        =self.density_heatmap,self.density_heatmap_points,self.density_heatmap_points_org,self.density_heatmap_surface,self.density_org_points 

        if save_data:
            with open(os.path.join(self.spine_path,'riplet.pkl'), 'wb') as f:
                pickle.dump(self.gsave, f) 
        else:
            return  self.gsave
        ...
        if tf_ripley:
            ...
            self.ripp,self.Lrip,intensity=ripley_all_save(save,R,**param_ripley)
            intensity['idx']={}
            self.others=dict(mesh_shaft=save['mesh_shaft'],
                             intensity=intensity,
                            )

            # rripp,pvals,p_cluster,p_regular,p_global=monte_carlos_ripley(save,R,**param_ripley,size_monte=size_monte,K_obs=self.ripp)
            # rrippp,pvalsa,p_clustera,p_regulara,p_global_area=monte_carlos_area(save,R=None,N_plot=40,rall=rall,N_interpolate=None,list_index=None,size_monte=size_monte,K_obs=self.ripp)
            ...
            res_ripley = monte_carlos_ripley_poisson_incremental(save, R, rall, size_monte=size_monte, K_obs=self.ripp)
            res_area   = monte_carlos_area_incremental(save, R=None, rall=rall, size_monte=size_monte, K_obs=self.ripp)
            self.res_ripley=res_ripley
            ...
            self.rall=param_ripley['rall']=rall
            print('[[[  lkkk ]]]',self.ripp)
            ...
            self.graph_ripp_norm=go.Scatter(
                    x=rall,
                    y=rripp,
                    mode='lines',
                    name='csr K-fun',
            ...
            self.graph_ripp_norm_area=go.Scatter(
                    x=rall,
                    y=rrippp,
                    mode='lines',
                    name='csr Area'
            ...
            self.graph_Lrip_area=go.Scatter(
                    x=rall,
                    y=self.ripp/(rrippp)-1,
                    mode='lines+markers',
                    name='L(r)-1,   Area'
                )
            ...
            self.graph_monte_ratio=go.Scatter(
                    x=rall,
                    y=rripp/(rrippp)-1,
                    mode='lines+markers',
            ...
            # pg=get_chi(pvals)
            pg=p_global[0]
            self.graph_pvals_2=go.Scatter(
                    x=rall,
                    y=pvals,
                    mode='lines+markers',
                    name=f'p-2 sides,  K-fun p:{pg:.3f}',
            ...
            # pg=get_chi(pvalsa)
            pg=p_global_area[0]
            self.graph_pvals_2_area=go.Scatter(
                    x=rall,
                    y=pvalsa,
                    mode='lines+markers',
                    name=f'p-2 sides,   Area p:{pg:.3f}',
                    marker=dict(
                        symbol='square',
                        size=8,
                        color='green'
            ...
            pg=p_global[1]
            self.graph_pvals_cluster=go.Scatter(
                    x=rall,
                    y=p_cluster,
                    mode='lines+markers',
                    name=f'p-cluster,  K-fun p:{pg:.3f}',
            ...
            pg=p_global_area[1]
            self.graph_pvals_cluster_area=go.Scatter(
                    x=rall,
                    y=p_clustera,
                    mode='lines+markers',
                    name=f'p-cluster,   Area p:{pg:.3f}',
            ...
            pg=p_global[2]
            self.graph_pvals_regular=go.Scatter(
                    x=rall,
                    y=p_regular,
                    mode='lines+markers',
                    name=f'p-regular,  K-fun p:{pg:.3f}',
                    marker=dict(
                        symbol='circle',
                        size=8,
                        color='purple'
            ...
            pg=p_global_area[2]
            self.graph_pvals_regular_area=go.Scatter(
                    x=rall,
                    y=p_regulara,
                    mode='lines+markers',
                    name=f'p-regular,   Area p:{pg:.3f}',
                    marker=dict(
                        symbol='circle',
                        size=8,
                        color='purple'
